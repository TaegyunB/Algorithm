...
public class Solution {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc=1; tc<=10; tc++) {
			...
			int[] pos = findStart(arr, 100);  // 시작지점 찾기 함수
			int x = pos[0];  // 시작 지점의 x
			int y = pos[1];  // 시작 지점의 y
			
			boolean[][] visited = new boolean[100][100];  // 방문 여부 확인 배열
			boolean found = false;
			...
			while(!deque.isEmpty()) {  // deque가 비어있지 않는 동안
				...
				for (int z=0; z<4; z++) {  // 우, 하, 좌, 상을 돌면서
					...
					if (nx >= 0 && nx < 100 && ny >= 0 && ny < 100 && arr[nx][ny] != 1 && !visited[nx][ny]) {  // 범위 안에 있고 벽이 아니고 방문한 적이 없다면
